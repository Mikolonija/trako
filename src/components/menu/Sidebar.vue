<template>
    <div :class="{ outside: sideBarStore.isMenuSideBarCollapsed }">
        <aside class="aside" @keyup.esc="sideBarStore.closeMenuSideBar()" tabindex="-1" :class="{
            collapsed: !sideBarStore.isMenuSideBarCollapsed,
            openSideBar: sideBarStore.isMenuSideBarOpen,
        }" ref="sidebar">
            <nav class="nav">
                <router-link to="/" class="nav-header" :class="{ collapsed: !sideBarStore.isMenuSideBarCollapsed }"
                    @click="sideBarStore.closeMenuSideBar()">
                    <SvgImage :alt="t('imagesAlt.emblemImg')" :iconSrc="emblemImg" :useComponent="false" width="36"
                        height="36" />
                    <h1 class="font-montserrat-36-bold" v-if="sideBarStore.isMenuSideBarCollapsed">
                        Trako
                    </h1>
                </router-link>
                <div class="menu">
                    ...
                    }" class="layout-left-center menu-container" active-class="active-link" @click="resetPage()">
                        <SvgImage :alt="t(item.alt)"
                            :iconSrc="item.activeLink.includes(route.name as string) ? item.activeImg : item.noActiveImg"
                            :useComponent="false" width="24" height="24" />
                        <span v-if="sideBarStore.isMenuSideBarCollapsed" class="menu-text font-montserrat-16-regular">
                            {{ t(item.text) }}
                        </span>
                    </router-link>
                </div>
            ...
            <div class="nav-footer" :class="{ collapsed: !sideBarStore.isMenuSideBarCollapsed }">
                <div class="nav-footer-position" :class="{ collapsed: !sideBarStore.isMenuSideBarCollapsed }">
                    <button type="button" :aria-label="t('buttonsLabels.toggleSidebar')" class="nav-footer-collapse-btn"
                        @click="sideBarStore.resizeMenuSideBar()" tabindex="0">
                        <SvgImage :alt="t('imagesAlt.arrowImg')" :iconSrc="arrowImg"
                            :class="{ collapsed: !sideBarStore.isMenuSideBarCollapsed }" name="item.text"
                            :useComponent="false" width="24" height="24" class="nav-footer-collapse-row-icon" />
                    </button>
                </div>
            </div>
        </aside>
...
const menuItems: NavigationList[] = [
    {
        ...
        noActiveImg: homeImg,
        activeLink: [
            routerPath.home.name,
            routerPath.homeByDate.name,
            routerPath.homeByDateRange.name,
        ],
    },
    {
