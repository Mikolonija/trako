<template>
    <div class="settings-container" :class="{ collapsed: !sideBarStore.isMenuSideBarCollapsed }">
        <div class="layout-center" v-if="!sideBarStore.isMenuSideBarCollapsed">
            <SvgImage :iconSrc="settingsImg" :alt="t('imagesAlt.settingsImg')" :useComponent="false" width="24"
                height="24" />
        </div>
        <template v-else>
        ...
        <ul class="settings-menu" :class="{ collapsed: !sideBarStore.isMenuSideBarCollapsed }">
            <li v-for="setting in settings" :key="setting.key" @click="sideBarStore.closeMenuSideBar()">
                <div class="layout-left-center settings-menu-header"
                ...
                <div class="setting-menu-lists">
                    <template v-for="option in setting.options" :key="option.value">
                        <p tabindex="0" @keydown.enter="setting.change(option.value)" :class="option.value === setting.active()
        ...
        options: [
            {
                ...
                label: !sideBarStore.isMenuSideBarCollapsed
                    ? ThemeType.DM
                    : t("menu.settings.darkMode"),
            },
            {
                value: ThemeType.LM,
                label: !sideBarStore.isMenuSideBarCollapsed
                    ? ThemeType.LM
                    : t("menu.settings.lightMode"),
            },
        ],
    },
    {
