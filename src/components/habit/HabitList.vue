<template>
    <div class="habit-list">
        ...
        <div class="habit-menu">
            <p class="habit-menu-item font-montserrat-16-regular" :class="{ 'active': habitStore.isHabitActive }"
                @click="changeSelection(HabitActivationStatus.Enable)">
                {{ t('habitList.selected.active') }}
            </p>
            <p class="habit-menu-item font-montserrat-16-regular" :class="{ 'active': !habitStore.isHabitActive }"
                @click="changeSelection(HabitActivationStatus.Disable)">{{ t('habitList.selected.inactive') }}
            </p>
        </div>
            ...
            <div class="habit-list-card">
                <template v-if="visibleHabits.length > 0">
                    <HabitCard :habit="habit" v-for="(habit, i) in visibleHabits" :key="i">
                        <template #actions>
                            <button type="button" :aria-label="t('buttonsLabels.enableHabit')"
                                class="cancel-btn-round layout-center" @keyup.esc="modalStore.hideModal()"
                                v-if="!habitStore.isHabitActive" @click="openEnableModal(habit)">
                                <SvgImage :alt="t('imagesAlt.unlockImg')" name="unlock" folder="habitCard"
                                    class="default-icon-color " />
                            </button>
                            <button type="button" :aria-label="t('buttonsLabels.disableHabit')"
                                @keyup.esc="modalStore.hideModal()" class="cancel-btn-round  layout-center"
                                v-if="habitStore.isHabitActive" @click="openDisableModal(habit)">
                                <SvgImage :alt="t('imagesAlt.lockImg')" name="lock" folder="habitCard"
                                    class="default-icon-color " />
                            </button>
                            <button type="button" :aria-label="t('buttonsLabels.editHabit')"
                                @keyup.esc="modalStore.hideModal()" v-if="habitStore.isHabitActive"
                                class="cancel-btn-round" @click="openEditModal(habit)">
                                <SvgImage :alt="t('imagesAlt.editImg')" name="edit" folder="habitCard"
                                    class="default-icon-color" />
                            </button>
                            <button type="button" :aria-label="t('buttonsLabels.deleteHabit')"
                                @keyup.esc="modalStore.hideModal()" class="cancel-btn-round"
                                @click="openDeleteModal(habit)">
                                <SvgImage :alt="t('imagesAlt.deleteImg')" name="delete" folder="habitCard"
                                    class="default-icon-color" />
                            </button>
                            <button type="button" :aria-label="t('buttonsLabels.markHabit')"
                                @keyup.esc="modalStore.hideModal()" class="cancel-btn-round layout-center"
                                v-if="shouldUpdateHabitStatus(habit, HabitStatus.Unfinished) && habitStore.isHabitActive"
                                @click="openMarkModal(habit)">
                                <SvgImage name="deselected" :alt="t('imagesAlt.deselectedImg')" folder="habitCard"
                                    class="default-checkbox-color" />
                            </button>
                            <button type="button" :aria-label="t('buttonsLabels.unmarkHabit')"
                                @keyup.esc="modalStore.hideModal()" class="cancel-btn-round layout-center"
                                v-if="shouldUpdateHabitStatus(habit, HabitStatus.Complete) && habitStore.isHabitActive"
                                @click="openUnmarkModal(habit)">
                                <SvgImage :alt="t('imagesAlt.selectedImg')" name="selected" folder="habitCard"
                                    class="default-checkbox-color" />
                            </button>
                        </template>
                    </HabitCard>
                ...
                <template v-else>
                    <div class="layout-center empty-habits-image-container">
                        <SvgImage :alt="t('imagesAlt.emptyImg')" @click="openCreateModal()" name="empty_habits"
                            folder="globals" class="empty-habits-image" />
                    </div>
                </template>
            </div>
        </div>
        <div class="load-more layout-center">
            <button type="button" :aria-label="t('buttonsLabels.loadMore')"
                class="loaded-more-btn font-montserrat-14-bold " v-if="habitStore.loadedMore < filteredHabits.length"
                @click="loadMore">
                {{ t('progress.loadMore') }}
            </button>
        </div>
...
const filteredHabits = computed(() => {
    const [startDate, endDate]: number[] = habitStore.dateRange.map(date => new Date(date).setHours(0, 0, 0, 0));
    return habitStore.habits
        .filter(habit => {
            const habitDate: number = new Date(habit.deadline).setHours(0, 0, 0, 0);
            return habitDate >= startDate && habitDate <= endDate && habit.isActive === habitStore.habitSelected;
        }).sort((a, b) => {
...
const openCreateModal = (): void => {
    ...
    modalStore.habitModalSubmitAction = (habitFormState: Habit) => habitStore.createHabit({
        ...habitFormState,
        id: uuid.v1(),
...
const openDisableModal = (habit: Habit): void => {
    ...
    modalStore.confirmModalSubmitAction = () => habitStore.changeHabitActivationStatus(habit, HabitActivationStatus.Disable);
...
const openEnableModal = (habit: Habit): void => {
    modalStore.component = markRaw(ConfirmModal);
    modalStore.confirmModalSubmitAction = () => habitStore.changeHabitActivationStatus(habit, HabitActivationStatus.Enable);
...
const openMarkModal = (habit: Habit): void => {
    modalStore.component = markRaw(ConfirmModal);
    modalStore.confirmModalSubmitAction = () => habitStore.changeHabitStatus(habit, HabitStatus.Complete);
    modalStore.bodyText = t('modal.confirmHabitStatusChange.text');
    modalStore.notificationSuccessDescription = t('modal.confirmHabitStatusChange.success.description');
...
const openUnmarkModal = (habit: Habit): void => {
    modalStore.component = markRaw(ConfirmModal);
    modalStore.confirmModalSubmitAction = () => habitStore.changeHabitStatus(habit, HabitStatus.Unfinished);
...
const openEditModal = (habit: Habit): void => {
    modalStore.component = markRaw(HabitModal);
    modalStore.habitModalSubmitAction = (habitFormState) => habitStore.updateHabit({
        ...habitFormState,
        name: cleanSpaces(habitFormState.name, SpaceCleanOption.TrimAndReplace),
    });
