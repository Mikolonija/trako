<template>
  ...
  >
    <div class="filter-sideBar-header">
      ...
      <button
        type="button"
        :aria-label="t('buttonsLabels.toggleHabitNavigationSidebar')"
        class="collapse-btn"
      ...
      >
        <SvgImage
          ...
          name="arrow"
          folder="globals"
          class="collapse-btn-row-icon"
          :class="{ collapsed: sideBarStore.isHabitNavigationSideBarCollapsed }"
        />
      </button>
    </div>
    <div
      class="filter-sideBar-main"
      :class="{ collapsed: sideBarStore.isHabitNavigationSideBarCollapsed }"
    >
      <VueDatePicker
        ...
        :markers="habitStore.markDays"
      />
      <button
        type="button"
        :aria-label="t('buttonsLabels.filterHabits')"
        :disabled="isSameDateRange(filterDateRange, habitStore.dateRange)"
        class="confirm-btn filter-btn-position font-montserrat-14-bold"
        @click="filterHabit()"
      >
      ...
      <button
        type="button"
        :aria-label="t('buttonsLabels.resetHabitFilter')"
        :disabled="isSameDateRange(filterDateRange, defaultHabitRange)"
        class="cancel-btn reset-btn-position font-montserrat-14-bold"
        @click="clearHabitFilter()"
      >
        {{ t('habitFilter.reset') }}
      </button>
    </div>
...
const isValidDateRange = (date: Date): boolean => {
  if (date < minHabitDate || date > maxHabitDate) {
    notify({
      ...
      text: t('notification.invalidDateRangeError.description', {
        minDate: formatDate(minHabitDate),
        maxDate: formatDate(maxHabitDate),
      }),
      type: 'error',
    });
    return false;
...
const filterHabit = (): void => {
  ...
  router.replace(
    `/day/${startDate.toISOString().split('T')[0]}/${endDate.toISOString().split('T')[0]}`,
  );
};
