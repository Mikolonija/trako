<template>
    <div class="form">
        <div class="form-container-space">
            ...
            <textarea @input="handleNameMaxLength()" ref="habitNameTextarea"
                :class="{ 'invalid-textarea': habitFormStore.timerErrorStyleState.name }"
                class="form-name-textarea font-montserrat-14-regular" id="habit-name" v-model="habitFormState.name"
                :placeholder="t('habitForm.placeholders.habitName')"></textarea>
        </div>
        <div class="form-container-space form-habit-container">
            <div class="form-container-space">
                <label class="font-montserrat-14-regular" for="start-date">{{
                    t('habitForm.labels.startDate') }}</label>
                <VueDatePicker prevent-min-max-navigation
                    ...
                    aria-labelledby="start-date" :min-date="minHabitDate" :max-date="habitFormState.endDate === null ?
                        maxHabitDate :
                        habitFormState.endDate
                        " :locale="locale.toLowerCase()" :enable-time-picker="false" v-model="habitFormState.startDate"
                    :cancel-text="t('calendar.btn.close')" :select-text="t('calendar.btn.pick')"
                    :dark="themeStore.getThemeStatus" placeholder="Select Date">
                    <template #clear-icon />
                </VueDatePicker>
            </div>
            <div class="form-container-space">
                <div class="form-habit-checkbox-container">
                    <label class="font-montserrat-14-regular" for="end-date">{{
                        t('habitForm.labels.endDate') }}</label>
                ...
                <VueDatePicker prevent-min-max-navigation
                    ...
                    :class="{ 'invalid-input': habitFormStore.timerErrorStyleState.endDate }" aria-labelledby="end-date"
                    :max-date="maxHabitDate" :min-date="habitFormState.startDate" :locale="locale.toLowerCase()"
                    :enable-time-picker="false" v-model="habitFormState.endDate" :cancel-text="t('calendar.btn.close')"
                    :select-text="t('calendar.btn.pick')" :dark="themeStore.getThemeStatus">
                    <template #clear-icon />
                </VueDatePicker>
            </div>
        </div>
        <div>
            ...
            <div class="day-list-container">
                <p class="day-list-container-card font-montserrat-14-regular" v-for="day in repeatDaysDefault"
                    :key="day.msg" :class="{
        ...
        <div class="form-container-space form-habit-container">
            <div class="form-container-space">
                <label class="font-montserrat-14-regular" for="start-time">{{
                    t('habitForm.labels.startTime') }}</label>
                <VueDatePicker :class="{ 'invalid-input': habitFormStore.timerErrorStyleState.timer }"
                    aria-labelledby="start-time" :cancel-text="t('calendar.btn.close')"
                    :select-text="t('calendar.btn.pick')" :locale="locale.toLowerCase()"
                    v-model="habitFormState.startTime" time-picker :dark="themeStore.getThemeStatus">
                    <template #clear-icon />
                </VueDatePicker>
            </div>
            <div class="form-container-space">
                <div class="form-habit-checkbox-container">
                    <label class="font-montserrat-14-regular" for="end-time">{{
                        t('habitForm.labels.endTime') }}</label>
                    <div class="layout-center">
                        <input type="checkbox" id="end-time" class="checkbox" v-model="isEndTime">
                    </div>
                </div>
                <VueDatePicker :disabled="habitFormState.endTime === null"
                    :class="{ 'invalid-input': habitFormStore.timerErrorStyleState.timer }" aria-labelledby="end-time"
                    :cancel-text="t('calendar.btn.close')" :select-text="t('calendar.btn.pick')"
                    :locale="locale.toLowerCase()" v-model="habitFormState.endTime" time-picker
...
const handleNameMaxLength = (): void => {
    ...
    let textWithoutSpaces: number = cleanSpaces(habitFormState.name, SpaceCleanOption.RemoveAll).length;
    if (textWithoutSpaces >= habitFormStore.nameMaxLength) habitNameTextarea.value.maxLength = habitFormState.name.length;
...
watch(() => habitFormState.name, () => {
    habitFormStore.checkNameError();
});

watch(() => habitFormState.startDate, () => {
    habitFormStore.checkDateError();
});

watch(() => habitFormState.endDate, () => {
    habitFormStore.checkDateError();
});

watch(() => habitFormState.startTime, () => {
    habitFormStore.checkTimeError();
});

watch(() => habitFormState.endTime, () => {
    habitFormStore.checkTimeError();
});
...
watch(isEndTime, (newValue: boolean) => {
    if (newValue) {
        if (oldEndTime.value !== null) habitFormState.endTime = oldEndTime.value;
            ...
            const startTime: Time = { hours: habitFormState.startTime.hours === 23 ? 23 : habitFormState.startTime.hours + 1, minutes: habitFormState.startTime.minutes };
            const endTime: Time = { hours: (startTime.hours) % 24, minutes: habitFormState.startTime.minutes };
