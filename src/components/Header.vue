    ...
    <header class="header-container">
        <div class="layout-left-center">
            <template v-if="!sideBarStore.isMenuSideBarOpen">
                <SvgImage :iconSrc="barImg" :alt="t('imagesAlt.barImg')" :useComponent="false" width="36" height="36"
                    @click="sideBarStore.openMenuSideBar()" />
            </template>
            <template v-else>
                <SvgImage :alt="t('imagesAlt.closeImg')" name="close" folder="globals" width="36" height="36"
                    class="default-icon-color" @click="sideBarStore.closeMenuSideBar()" />
            </template>
        </div>
...
.header-container {
    ...
    box-shadow: var(--header-shadow-color) 0px 14px 28px,
